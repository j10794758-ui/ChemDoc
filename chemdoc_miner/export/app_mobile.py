...
<style>
:root{--bg:#0f1419;--card:#1a2332;--text:#e8eef5;--muted:#8fa3b8;--accent:#4db6ff;--accent2:#f0a030;--warn:#f0a030;--danger:#ff6b6b;--line:#2a3544;--ok:#6bcb77;--nav-h:56px}
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--text);min-height:100dvh;padding:16px 16px calc(var(--nav-h) + 16px)}
h1{font-size:1.2rem;font-weight:700;margin-bottom:4px}
.sub{color:var(--muted);font-size:.82rem;margin-bottom:12px}
.view{display:none}.view.on{display:block}
.app-nav{position:fixed;left:0;right:0;bottom:0;height:var(--nav-h);padding-bottom:env(safe-area-inset-bottom);display:flex;background:var(--card);border-top:1px solid var(--line);z-index:100}
.app-nav button{flex:1;border:none;background:transparent;color:var(--muted);font-size:.78rem;font-weight:600;padding:8px 4px}
.app-nav button.on{color:var(--accent);box-shadow:inset 0 2px 0 var(--accent)}
.search{display:flex;gap:8px;margin-bottom:12px}
input,select{background:var(--card);border:1px solid var(--line);color:var(--text);border-radius:10px;padding:12px;font-size:16px}
input[type=text],input[type=number]{flex:1}
select{flex:1;min-width:120px}
button.action{background:var(--accent);color:#111;border:none;border-radius:10px;padding:12px 16px;font-weight:700;font-size:16px}
button.primary{width:100%;margin-top:4px}
button.link{background:transparent;border:none;color:var(--accent);font-size:.82rem;padding:8px 0;cursor:pointer}
button.link-btn{background:transparent;border:1px solid var(--line);color:var(--accent);border-radius:8px;padding:6px 10px;font-size:.78rem;cursor:pointer}
...
.checks label{font-size:.82rem;display:flex;gap:8px;align-items:flex-start}
.card{background:var(--card);border:1px solid var(--line);border-radius:12px;padding:12px 14px;margin-top:10px;overflow:hidden}
...
.reasons{margin-top:8px;font-size:.78rem;line-height:1.4}
...
.back-bar button{background:#243044;border:1px solid var(--line);color:var(--accent);border-radius:8px;padding:8px 12px;font-size:.85rem;font-weight:600;width:100%;text-align:left}
.tabs{display:flex;border-bottom:1px solid var(--line)}
.tab{flex:1;padding:12px;text-align:center;font-size:.88rem;color:var(--muted);cursor:pointer;border:none;background:transparent}
...
.section p,.section li{font-size:.88rem;line-height:1.45}
.section ul{padding-left:18px}
.kv{display:grid;grid-template-columns:minmax(110px,38%) 1fr;gap:6px 10px;font-size:.84rem}
.kv div:nth-child(odd){color:var(--muted)}
.pcode,.hcode{font-size:.82rem;padding:6px 0;border-bottom:1px solid rgba(42,53,68,.6)}
.code{font-family:ui-monospace,monospace;color:var(--warn);margin-right:8px}
.ghs-label{background:#fff;color:#111;border:2px solid #c8102e;border-radius:4px;padding:12px;margin-bottom:14px}
...
.hrow{display:flex;justify-content:space-between;padding:12px 14px;border-bottom:1px solid var(--line);gap:12px}
.results{background:var(--card);border:1px solid var(--line);border-radius:12px;overflow:hidden}
.row{display:flex;justify-content:space-between;padding:12px 14px;border-bottom:1px solid var(--line);align-items:center;gap:8px}
.row:last-child{border-bottom:none}
.row button.link-btn{flex-shrink:0}
.co{font-weight:600}
.grade{color:var(--accent2);font-family:ui-monospace,monospace}
.note{padding:10px 14px;font-size:.82rem;color:var(--muted);border-bottom:1px solid var(--line)}
.section-hdr{padding:8px 14px;font-size:.75rem;color:var(--accent2);background:rgba(240,160,48,.08)}
